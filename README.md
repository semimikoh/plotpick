...
2. **스트리밍 문제** -- LLM 응답이 들어올 때마다 컨테이너 높이 변경
3. **가상화 불가** -- 화면 밖 메시지의 높이를 모르면 스크롤 위치 계산 불가
...
```typescript
function estimateMessageHeight(message: ChatMessage, containerWidth: number): number {
  let height = 0;
  ...
  // 장르 버튼: 버튼 폭 누적 -> 줄 넘침 시 행 증가
  if (message.genreOptions) {
    height += calculateButtonRows(message.genreOptions, containerWidth) * 38;
  }
